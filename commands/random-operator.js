...
module.exports.run = async (interaction, client, db) => {
    ...
    async function checkOperator (operator) {
        ...
        if (Array.isArray(usedOperators) && operatorList.includes(operator) && usedOperators.includes(operator)) return true;
        else return false;
    }
...
module.exports.command = new SlashCommand()
    ...
    .setDescription('Get a random operator from the selected team')
